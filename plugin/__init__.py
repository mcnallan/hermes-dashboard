    ...
    with _APPROVALS_LOCK:
        ...
        for item in _APPROVALS.values():
            ...
            if item.get("resolved"):
                continue
            if item.get("session_key") == session_key and item.get("command") == command:
                return item
    return None
...
def _handle_approval_control(payload):
    ...
    _send(_base_payload(
        ...
        approval.get("session_id") or approval.get("session_key") or _CURRENT_SESSION_ID or approval_id,
...
def _dashboard_approval_callback(command, description, *, allow_permanent=True):
    ...
    if not event.wait(timeout=_approval_timeout()):
        ...
        _send(_base_payload(
            "ApprovalResponse",
            approval.get("session_id") or approval.get("session_key") or _CURRENT_SESSION_ID or approval["approval_id"],
            "processing",
...
def _create_approval_request(command="", description="", pattern_key="", pattern_keys=None, session_key="", surface="", **kwargs):
    ...
    session_id = kwargs.get("session_id") or session_key or _CURRENT_SESSION_ID or approval_id
    approval = {
        "approval_id": approval_id,
        "session_id": session_id,
...
def _on_post_approval_response(command="", description="", pattern_key="", pattern_keys=None, session_key="", surface="", choice="", **kwargs):
    session_id = kwargs.get("session_id") or session_key or _CURRENT_SESSION_ID or ""
    approval = _find_approval(session_key or session_id, command, description)
